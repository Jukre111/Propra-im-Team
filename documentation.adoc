...
_Bei Start der Anwendung werden mit Hilfe des Databaseinitializers User erzeugt.
...
* In jeden Commit schreiben, welche zwei Leute beteiligt waren (Vornamen).
...
* Von der Übersichtsseite gelangt man zu einer Detailansicht jedes einzelnen
Artikels, indem man auf dessen Namen oder Bild klickt. [green big]#✔#
** Die Detailansicht der ausleihbaren Artikel enthält:
Name und Beschreibung des Artikels, Angaben zu Tagessatz und Kaution,
...
zur verleihenden Person (username), Angaben zum Standort des Artikel (die
  ...
  [green big]#✔#
* Verleihbare Artikel können neu eingestellt werden mit (Button "Verleihen" auf der
Startseite):
** Name, Artikelbeschreibung, Foto des Artikels(muss nicht zwangsläufig
  vorhanden sein), Kautionswert, Tagessatz
[green big]#✔#
* Käufliche Artikel können neu eingestellt werden mit (Button "Verkaufen" auf der
Startseite"):
...
** Es können nur Daten ab dem aktuellen Tag gewählt werden und das Enddatum darf
nicht vor dem Startdatum liegen (sonst -> Warnung und anfragen-Button
  disabled).
...
[green big]#✔#
* Werden Artikel in gutem Zustand zurückgegeben, so kann die verleihende Person
...
bestätigt.
** Wird der Artikel zu spät, gar nicht oder defekt zurückgegeben, so kann die
...
können auf der Propayaccount-Seite eingesehen werden (über die account-Seite
...
werden. Da in jedem Conflict eine Liste von Messages gespeichert wird, können
...
bestätigt werden, dass der entsprechende Gegenstand rechtzeitig und unversehrt
...
* Für das bearbeiten der Angaben wird dasselbe Template verwendet.
